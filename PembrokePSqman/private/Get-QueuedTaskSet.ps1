function Get-QueuedTaskSet {
    ...
    if (Test-Connection -Count 1 $RestServer -Quiet) {
        try
        {
            ...
            $SubmittedTasks = (Invoke-RestMethod -Method Get -Uri "http://$RestServer/PembrokePS/public/api/api.php/$TableName?filter=STATUS_ID,eq,6&transform=1" -UseBasicParsing).$TableName
        }
        catch
        {
