...
Set-Location C:\OPEN_PROJECTS\ProjectPembroke\PembrokePSqman
...
foreach ($Directory in $Directories)
{
    ...
    $files = Get-ChildItem .\PembrokePSqman\$Directory
    foreach ($file in $files)
    {
        # Source the file
        $FileName = $file.Name
        Write-Output "Importing file $FileName"
        . .\PembrokePSqman\$Directory\$FileName
    }
    # Execute Pester for the Directory.
    $results = Invoke-pester .\tests\$Directory\*.ps1 -CodeCoverage .\PembrokePSqman\$Directory\*.ps1 -PassThru
